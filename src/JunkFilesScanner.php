...
class JunkFilesScanner extends CI_Controller {
	...
	* @return
	*/
    public function find_junk_records($type = NULL){
        ini_set('memory_limit', '2048M');
    	$ussege = array();
		...
		$ngFiles = $jsFiles = $cssFiles = $appFiles = array();
		$allMergedFiles = array();
    ...
    private function list_folders_from_dir($folderPath) {
        ...
        $folders = array();
        $allFilesFolders = get_dir_file_info($folderPath,TRUE, TRUE);
    ...
    private function list_all_files_from_dir_top_level($folder) {
        ...
        $allFilesFolders = get_dir_file_info($folder,TRUE, TRUE);
        foreach($allFilesFolders as $key => $value){
			if(!is_dir($value['server_path'])){
				$files[] = $value;
			}
    ...
    private function list_all_files_from_dir_bottom_level($folder, $includeFolders = FALSE) {
        $files_array = get_filenames($folder, $includeFolders);
        return $files_array;
    }
	...
	*/
    ...
    private function list_files_in_folder($folder, $directoryLevel = -1) {
        ...
        $files_array = directory_map('./' . $folder, $directoryLevel);
        return $files_array;
    }
        ...
        if($listFolders && $target_files){
			foreach($listFolders as $sourceDir){
		        ...
		        foreach($target_files as $target_file) {
		            ...
		            $target_file_content = read_file($target_file_full);
	...
	* @return
	*/
    private function files_usage_in_group_of_files($sourceDirs = NULL, $target_files = array(), $includeDatabase = FALSE) {
        $innerMatches = array();
        $innerFiles = array();
        $usage = array();
        $filesMatchedArray = array();
        $totalMatched = 0;
	...
	* @return Array $usage
	*/
    private function single_file_usage_in_group_of_files($innerFile = NULL, $target_files = array(), $includeDatabase = FALSE) {
        $innerMatches = array();
        $innerFiles = array();
        $usage = array();
